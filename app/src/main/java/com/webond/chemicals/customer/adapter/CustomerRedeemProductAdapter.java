...
public class CustomerRedeemProductAdapter extends RecyclerView.Adapter<CustomerRedeemProductAdapter.MyViewHolder> {
    ...
    private ICustomerOnRedeemPoint iCustomerOnRedeemPoint;

    public CustomerRedeemProductAdapter(Context context, ArrayList<GetRedemProductListPojo> getRedemProductListPojoArrayList) {
        this.context = context;
        this.getRedemProductListPojoArrayList = getRedemProductListPojoArrayList;
        layoutInflater = LayoutInflater.from(context);
        mySharedPreferences = new MySharedPreferences(context);
        this.iCustomerOnRedeemPoint = (ICustomerOnRedeemPoint) context;
    ...
    public MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = layoutInflater.inflate(R.layout.customer_redeem_product_list_item, parent, false);
        return new MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull CustomerRedeemProductAdapter.MyViewHolder holder, int position) {
...
//                                getRedemProductListPojo.getSchemeProductPoints(),
//                                holder.tvProductPoint);
//                    } else {
//                        holder.etQty.setText(holder.etQty.getText().toString());
//                        holder.tvProductPoint.setText(holder.tvProductPoint.getText().toString()+"");
//                        Toast.makeText(context, "Insufficient points Test", Toast.LENGTH_SHORT).show();
//                    }
//                } catch (Exception ex) {
//                    ex.printStackTrace();
//                    System.out.print("TEST:____:- " + ex.getMessage());
//                }
//            }
//        });
...
//    void addQuantity(int addBy, AppCompatEditText etQty, TextViewMediumFont tvProductPoint) {
//
//        int avlQty = 0;
//
//        if (!CommonUtil.checkIsEmptyOrNullCommon(etQty.getText().toString())) {
...
//    }
    ...
    private void redeemProductApiCall(String RedemProductId, String RedemPoint, String Qty) {
        ...
        ApiImplementer.SaveRedeemData(RedemProductId, RedemPoint, "4", mySharedPreferences.getCustomerId(), "0", "0", Qty, new Callback<ArrayList<SaveRedemDataPojo>>() {
            @Override
                    ...
                    if (response.code() == 200 && response.body() != null) {
                        if (response.body().size() > 0) {
                            ...
                            if(saveRedemDataPojos.getStatus() == 1){
                                iCustomerOnRedeemPoint.onRedeemPoints(RedemPoint);
...
//                                iRedeemProductCustomer.productRedeemed();
                            }else{
    ...
    public interface ICustomerOnRedeemPoint{
